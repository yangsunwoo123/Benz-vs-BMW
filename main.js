/* ═══════════════════════════════════════════════════════
   main.js — Mercedes-Benz vs BMW Brand Comparison
═══════════════════════════════════════════════════════ */
  ...
  function draw() {
    ...
    for (var i = 0; i < particles.length; i++) {
      ...
      for (var j = i + 1; j < particles.length; j++) {
        var q = particles[j];
        var ex = p.x - q.x, ey = p.y - q.y, d = Math.sqrt(ex*ex + ey*ey);
        if (d < 100) {
          ctx.beginPath(); ctx.moveTo(p.x, p.y); ctx.lineTo(q.x, q.y);
          ctx.globalAlpha = (1 - d/100) * 0.12; ctx.strokeStyle = p.color;
...
function goToSlide(index) {
  ...
  isAnimating = true;
  var fromEl = slides[currentSlide], toEl = slides[index];
  ...
  fromEl.style.transition = toEl.style.transition = 'transform ' + ANIM_MS + 'ms cubic-bezier(0.77,0,0.175,1)';
  fromEl.style.transform = dir > 0 ? 'translateX(-100%)' : 'translateX(100%)';
  toEl.style.transform   = 'translateX(0)';
  closeAllDetails();
  setTimeout(function () {
    fromEl.classList.remove('active'); fromEl.style.transition = ''; fromEl.style.transform = '';
    toEl.classList.add('active'); toEl.classList.remove('transitioning'); toEl.style.transition = '';
    currentSlide = index; isAnimating = false;
    updateNavState(); triggerSlideIn(currentSlide);
  }, ANIM_MS);
}
...
/* ── SLIDE-IN ANIMATIONS ─────────────────────────────── */
var barsAnimated = {}, priceAnimated = false, marketAnimated = false;

function triggerSlideIn(index) {
  var slide = slides[index];
  if (!slide) return;

  if (!barsAnimated[index] && slide.querySelectorAll('.pbr-fill').length > 0) {
    barsAnimated[index] = true;
    setTimeout(function() {
  ...
  if (slide.dataset.animate === 'market' && !marketAnimated) {
    marketAnimated = true;
    setTimeout(function() {
      slide.querySelectorAll('.mbar-fill').forEach(function(bar) {
        bar.style.width = bar.dataset.w + '%';
      });
    }, 400);
  }
...
document.addEventListener('touchend', function (e) {
  ...
  if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 50) goToSlide(currentSlide + (dx > 0 ? 1 : -1));
}, { passive: true });
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* TOC items navigate to target slide */
  document.querySelectorAll('.toc-item[data-target]').forEach(function(item) {
    item.addEventListener('click', function() {
      var t = parseInt(item.dataset.target);
      if (!isNaN(t)) goToSlide(t);
    });
  });

  /* Feature tabs — scoped to parent slide */
  document.querySelectorAll('.ftab').forEach(function(btn) {
    btn.addEventListener('click', function() {
      var target = btn.dataset.tab;
      var slide = btn.closest('.slide');
      if (!slide) return;
      slide.querySelectorAll('.ftab').forEach(function(b) { b.classList.remove('active'); });
      slide.querySelectorAll('.feat-panel').forEach(function(p) { p.classList.remove('active'); });
  ...
  document.querySelectorAll('.fitem.clickable, .iitem.clickable').forEach(function(item) {
    item.addEventListener('click', function() {
      ...
      if (car === 'benz') {
        badge.textContent = '✦ Mercedes-Benz'; badge.className = 'fd-car-badge fd-benz';
      } else if (car === 'bmw') {
        badge.textContent = '◎ BMW'; badge.className = 'fd-car-badge fd-bmw';
      } else {
        badge.textContent = '비교 분석'; badge.className = 'fd-car-badge';
      }
  ...
  document.querySelectorAll('.fd-close').forEach(function(btn) {
    btn.addEventListener('click', closeAllDetails);
  });
});

function closeAllDetails() {
  document.querySelectorAll('.feature-detail').forEach(function(p) { p.classList.remove('open'); });
  document.querySelectorAll('.fitem-active, .iitem.fitem-active').forEach(function(el) { el.classList.remove('fitem-active'); });
}

/* ── FULLSCREEN ──────────────────────────────────────── */
var FS_EXPAND   = '<polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/>';
var FS_COLLAPSE = '<polyline points="4 14 10 14 10 20"/><polyline points="20 10 14 10 14 4"/><line x1="10" y1="14" x2="3" y2="21"/><line x1="21" y1="3" x2="14" y2="10"/>';
...
document.addEventListener('fullscreenchange', function () {
  ...
  if (!btn || !icon) return;
  if (document.fullscreenElement) {
    btn.title = '전체화면 해제 (Esc)'; icon.innerHTML = FS_COLLAPSE;
  } else {
    btn.title = '전체화면'; icon.innerHTML = FS_EXPAND;
  }
});
